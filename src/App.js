          ...
          {tab==="결산" && (subTab==="주문조회"||subTab==="결산조회") && (
            <>
              ...
              <div style={{...card, marginBottom:14, padding:"16px 18px"}}>
                ...
                <div style={{fontSize:11,color:"#94A3B8",marginBottom:8}}>🔍 DEBUG — brandId: {filter.brandId||"(전체)"} / mallType: {filter.mallType||"(전체)"} / {filter.from}~{filter.to} / filtered:{filtered.length}건 / orders전체:{orders.length}건</div>
