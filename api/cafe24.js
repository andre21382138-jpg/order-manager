module.exports = async (req, res) => {
  ...
  if (action === "token") {
    ...
    try {
      ...
      while (true) {
        const r = await fetch(
          `https://${mall_id}.cafe24api.com/api/v2/admin/orders?shop_no=1&start_date=${start_date}&end_date=${end_date}${mall_id === "afrimo" ? "&date_type=order_date" : ""}&limit=${pageSize}&offset=${offset}&embed=items`,
          { headers }
        );
